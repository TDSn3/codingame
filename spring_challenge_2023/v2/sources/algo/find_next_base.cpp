...
/*   Updated: 2023/06/04 23:46:01 by tda-silv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (!bfs_queue.empty())
	{
		...
		if (dist < max_dist)
		{
			...
			for (int j = 0; j < 6; j++)
			{
				...
				if (neighbor != -1 && !visited[neighbor] && stock_data.check_opp_base(neighbor) == -1 && (stock_data.data_of_cells[neighbor][BEACON_THIS_LOOP] == 1))
				{
