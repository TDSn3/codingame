...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   find_next_opp_base_connected_ant.cpp               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tda-silv <tda-silv@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/04 15:27:08 by tda-silv          #+#    #+#             */
/*   Updated: 2023/06/05 00:56:07 by tda-silv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::vector<int>	find_next_opp_base_connected_ant(Data& stock_data, int origin, int max_dist)
{
	...
	while (!bfs_queue.empty())
	{
		...
		if (dist < max_dist)
		{
			if (stock_data.check_opp_base(index) > -1)
			{
				std::vector<int>	path;
				int					current;

				current = stock_data.check_opp_base(index);
			...
			for (int j = 0; j < 6; j++)
			{
				...
				if (neighbor != -1 && !visited[neighbor] && (stock_data.data_of_cells[neighbor][OPP_ANTS] > 0))
				{
